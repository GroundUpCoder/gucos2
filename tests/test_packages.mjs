...
try{
  ...
  for(const name of [...declared.publishedDefinitions,...(declared.externalDefinitionSources||[]),...declared.siblingToolchainDefinitions,...declared.publishedSourceCompanions])check(`catalog contains ${name}`,new RegExp(`(^|\\s)${name}(\\s|$)`,'m').test(catalog));
  ...
  check('cpython-clang installs and owns the python role',out.includes('CPINSTALL=0')&&out.includes('{"python": "ok"}')&&out.includes('CPRUN=0'));
