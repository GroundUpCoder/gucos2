...
export default function TermPage() {
  ...
  return <div className="flex-1 min-h-0 flex flex-col" data-testid="term-page">
    ...
    <div id="tabs" className="flex gap-1 overflow-x-auto px-2 py-1.5 border-b bg-card">
      {sessions.map(s => <button key={s.id} className={`px-2 py-1.5 rounded text-sm whitespace-nowrap ${terminalStore.active === s.id ? 'bg-accent' : 'text-muted-foreground'}`} onClick={() => terminalStore.activate(s.id)} onDoubleClick={() => setRenaming(s.id)}><span>{s.name}</span><span data-testid="terminal-close" className="ml-2 text-destructive" onClick={e => { e.stopPropagation(); terminalStore.close(s.id); }}>×</span></button>)}
    ...
    <div className="relative flex-1 min-h-0 bg-[#080b12]">{sessions.map(s => <XtermView key={s.id} session={s} active={s.id === terminalStore.active} fontSize={font} />)}</div>
    <div id="keys" className="flex gap-1 overflow-x-auto p-1.5 bg-card safe-bottom md:hidden">{KEYS.map(([label,data]) => <button key={label} className="min-w-11 px-2 py-2 border rounded" onClick={() => input(data)}>{label}</button>)}</div><Dialog open={renaming!==null} title="Rename terminal" initial={renaming!==null?terminalStore.sessions.get(renaming)?.name:''} onCancel={()=>setRenaming(null)} onConfirm={v=>{if(renaming!==null&&v)terminalStore.rename(renaming,v);setRenaming(null);}} />
  </div>;
}
