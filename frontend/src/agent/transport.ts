...
export async function streamMessages(profile: EndpointProfile, model: string, messages: unknown[], tools: unknown[], signal: AbortSignal, callbacks: StreamCallbacks = {}, system?: string): Promise<{ content: Record<string,unknown>[]; stop_reason: string|null; usage: Record<string,unknown> }> {
  ...
  try {
    ...
    const reader = response.body.getReader(), decoder = new TextDecoder(); let buffer = '', stop_reason: string|null = null, usage: Record<string,unknown> = {}, blocks: Record<string,unknown>[] = [];
    for (;;) {
      const {done,value} = await reader.read(); if (done) break; arm(); buffer += decoder.decode(value, { stream: true });
      for (;;) { const split = buffer.search(/\r?\n\r?\n/); if (split < 0) break; const raw = buffer.slice(0, split); buffer = buffer.slice(split).replace(/^\r?\n\r?\n/, '');
        ...
        else if (event.type === 'message_delta') { stop_reason = String(event.delta?.stop_reason ?? stop_reason); usage = { ...usage, ...event.usage }; }
        else if (event.type === 'message_start') usage = event.message?.usage ?? {};
      }
    }
