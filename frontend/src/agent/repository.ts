...
export async function deleteThread(thread: ThreadSummary): Promise<void> { await enqueue(thread.path,async()=>{await gucFs.remove(thread.path);seqs.delete(thread.path)});await rebuildIndex(); }
