...
#else
/* PATCH(c-compiler): this compiler ignores `#pragma pack`, so the packed GB
   savestate struct sizes differently. This is the Game Boy savestate ABI —
   unused in this GBA-only build (GBSerializedState is only pulled in
   transitively via gb/audio.c for GBA's PSG audio path; GBASerializedState
   uses natural alignment + explicit padding and its assert holds). Neither
   GBSerialize/GBDeserialize is called from the mGBA frontend. */
#endif
