...
#ifdef __MTOTS__
/* PATCH(c-compiler): this C->wasm compiler has no __attribute__((constructor))
   and no pre-main ctor pass. The only CONSTRUCTOR users here are the
   mLOG_DEFINE_CATEGORY log-category registrars; dropping the constructor
   leaves each category id at 0 (all logs share the default category — a
   cosmetic loss). Define the function so `CONSTRUCTOR(FN) { ... }` still
   compiles; it is simply never called. */
#define CONSTRUCTOR(FN) ATTRIBUTE_UNUSED static void FN(void)
#else
