    ...
    switch (gb->printer.command_id) {
        ...
        case GB_PRINTER_START_COMMAND:
            if (gb->printer.command_length == 4) {
                ...
                /* PATCH(c-compiler): VLA replaced — image_offset is clamped to
                   sizeof(gb->printer.image) == 160*200; static to spare the
                   64KB wasm stack (single-threaded world) */
                static uint32_t image[160 * 200];
