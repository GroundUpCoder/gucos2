/* SameBoy boot ROMs, embedded as C arrays.
 *
 * Binary provenance: dmg_boot.bin (256 B) and cgb_boot.bin (2304 B) from
 * the official SameBoy v1.0.3 release artifact sameboy_winsdl_v1.0.3.zip
 * (github.com/LIJI32/SameBoy, MIT license) — SameBoy's own boot ROM
 * reimplementations, built from BootROMs/*.asm with rgbds. Embedded as
 * arrays because this repo has no rgbds toolchain and the standalone
 * build (sameboy.html) has no filesystem to load them from.
 */

#include "bootroms.h"

const unsigned char dmg_boot[256] = {
...
const unsigned char cgb_boot[2304] = {
