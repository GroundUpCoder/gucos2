...
static void LoadBoard( BOARD *p_board )
{
    ...
    if (RegQueryValueExW(hkey, u"Sound" /* ILP32 port: L"" is 4-byte here; u"" is WCHAR */, NULL, &type, (BYTE*)&p_board->IsSoundEnabled, &size) == ERROR_SUCCESS)
...
static void SaveBoard( BOARD *p_board )
{
    ...
    RegSetValueExW(hkey, u"Sound" /* ILP32 port: L"" is 4-byte here; u"" is WCHAR */, 0, REG_DWORD, (LPBYTE)&p_board->IsSoundEnabled, sizeof(p_board->IsSoundEnabled));
#endif
