...
/* gucOS wasm port: this repo's preprocessor does not re-scan a function-like
   macro name produced by an inner expansion when its `(...)` args come from the
   SAME replacement list. An extra expansion pass (JV_PP_EXPAND) forces the
   rescan so the selected JV_ARRAY_N/JV_OBJECT_N is invoked. */
#ifndef JV_PP_EXPAND
#define JV_PP_EXPAND(x) x
#endif
#define JV_ARRAY_IDX(_1,_2,_3,_4,_5,_6,_7,_8,_9,NAME,...) NAME
#define JV_ARRAY(...) \
  JV_PP_EXPAND(JV_ARRAY_IDX(__VA_ARGS__, JV_ARRAY_9, JV_ARRAY_8, JV_ARRAY_7, JV_ARRAY_6, JV_ARRAY_5, JV_ARRAY_4, JV_ARRAY_3, JV_ARRAY_2, JV_ARRAY_1, dummy)(__VA_ARGS__))
...
#else
/* gucOS wasm port: compilers without __GNUC__ (this repo's compiler.js) get
   no format-checking attribute — the macros must still expand to nothing so
   the declarations that use them parse. Upstream lacks this #else branch. */
#define JV_PRINTF_LIKE(fmt_arg_num, args_num)
#define JV_VPRINTF_LIKE(fmt_arg_num)
#endif
...
#define JV_OBJECT(...) \
  JV_PP_EXPAND(JV_OBJECT_IDX(__VA_ARGS__, \
