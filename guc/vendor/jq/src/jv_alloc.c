...
/* gucOS wasm port: this repo's libc returns NULL for a zero-size malloc/calloc
   (C11 6.7.6.2 permits either NULL or a unique pointer). jq's guarded
   allocators — and their callers, e.g. a bytecode block with zero
   subfunctions calling jv_mem_calloc(0, ...) — treat NULL as fatal OOM,
   assuming the glibc-style non-NULL-for-zero behavior. Normalize a zero-size
   request to one byte so the invariant holds. */
...
void* jv_mem_calloc(size_t nemb, size_t sz) {
  void* p = (nemb && sz) ? calloc(nemb, sz) : calloc(1, 1);
  if (!p) {
    memory_exhausted();
  }
  return p;
...
void* jv_mem_realloc(void* p, size_t sz) {
  p = realloc(p, sz ? sz : 1);   /* gucOS: avoid realloc(p,0) NULL == fatal */
  if (!p) {
    memory_exhausted();
  }
  return p;
