...
jv jv_load_file(const char* filename, int raw) {
  ...
  // before appending.
  /* gucOS wasm port: a `const int` size is not an integer constant expression
     in C, so `char buf[4096+max_utf8_len]` is a VLA (unsupported here). Use an
     enum constant so the array bound is a true compile-time constant. */
  enum { max_utf8_len = 4 };
