...
static void re_wrap(void)
{
	...
	int new_cur_fline = 0;
	uint32_t lineno;
	/* WASM PORT PATCH: VLA -> heap (no VLAs in this compiler); freed at
	 * the single exit below. */
	char *linebuf = xmalloc(w + 1);
...
static void print_found(const char *line)
{
	...
	/* WASM PORT PATCH: VLA -> heap (no VLAs in this compiler). */
	char *buf = xmalloc(width + 1);
	const char *str = line;
	char *p = buf;
	size_t n;
