...
#endif
{
	 ...
	 * (wasm_port.h; same rewrite as hush's three sites). BB_MMU is 0 on
	 * this platform by construction, so the journaled "child" re-execs
	 * the standalone unzipper applet (`gunzip -cf -`), NOMMU-Linux
	 * style; exec failure dies via pv_execvp's -1 -> perror_and_die. */
