...
#  endif
	 ...
	 * (wasm_port.h; same rewrite as hush's three sites), and execlp ->
	 * execvp (this libc has no execlp; the intercept macros only route
	 * the execv* family). */
...
# undef showopt
	...
	/* WASM PORT PATCH: guard the whole block — with the feature off,
	 * OPT_2COMMAND is 0 and the block is dead, but the address-taken
	 * data_extract_to_command reference survives this compiler's DCE
	 * (function-pointer stores emit table entries; only dead CALLS are
	 * dropped, see the date.c precedent). */
#if ENABLE_FEATURE_TAR_TO_COMMAND
	if (opt & OPT_2COMMAND) {
