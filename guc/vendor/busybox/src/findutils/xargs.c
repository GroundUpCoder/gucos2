 ...
 */
/* WASM PORT PATCH: statement expression -> ALWAYS_INLINE helper (no GNU
 * statement exprs in this compiler; same rewrite as libbb.h's ctype trio). */
static ALWAYS_INLINE int ISSPACE(unsigned char a)
{
	unsigned char xargs__isspace = a - 9;
	return xargs__isspace == (' ' - 9) || xargs__isspace <= (13 - 9);
}
