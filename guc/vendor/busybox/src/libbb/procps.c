...
/* WASM PORT PATCH: the uid/gid->name cache calls uid2uname_utoa (libpwdgrp)
 * — on this single-user system port/libbb_stubs.c answers "root" without a
 * cache (and supplies the clear_username_cache no-op), the same
 * FEATURE_LS_USERNAME-off philosophy as ls/tar. */
#ifndef __wasm__
...
#endif /* !__wasm__ (WASM PORT PATCH) */
