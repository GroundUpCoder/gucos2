...
# endif
				...
				/* WASM PORT PATCH: this libc's RAND_MAX is 32767 (15
				 * bits); compose 63 uniform bits from five draws
				 * (upstream only handles >=31-bit RAND_MAX). */
				uint64_t v = 0;
				int rnd_i;
				for (rnd_i = 0; rnd_i < 5; rnd_i++)
					v = (v << 15) ^ (unsigned)rand();
				v &= 0x7fffffffffffffffULL;
				R_d = (double)v / 0x8000000000000000ULL;
#else
