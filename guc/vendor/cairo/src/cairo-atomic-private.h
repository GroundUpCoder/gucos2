...
/* WASM PATCH: callers pass typed object-pointer lvalues (pixman_image_t **);
 * cast to the impl's cairo_atomic_intptr_t * as the HAS_ATOMIC_OPS variants
 * effectively do. */
#define _cairo_atomic_ptr_cmpxchg_return_old(x, oldv, newv) _cairo_atomic_ptr_cmpxchg_return_old_impl ((cairo_atomic_intptr_t *) (x), oldv, newv)
